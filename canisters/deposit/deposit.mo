...
shared (msg) actor class Deposit(
    ...
    public func getCurrentMultiplierBaseOnTimestamp(userInfo : DepositType, time : Time.Time) : async Float {
        var decayPerDay : Float = await getDecayPerDay(userInfo);
                ...
                for (deposit in deposits.vals()) {
                    for (reward in rewardDepositInfo.vals()) {
                        if (reward.depositTime >= timeThreshold and reward.depositTime > deposit.lastClaimedTime and reward.depositTime > deposit.startTime) {
                            ...
                            let withdrawValue = Float.fromInt(reward.amount) * (currentMul / Float.fromInt(reward.totalShare));
                            totalInterest += Nat64.toNat(Int64.toNat64(Float.toInt64(withdrawValue)));
                        };
                    };
                };
            };
            ...
            for (deposit in deposits.vals()) {
                for (reward in rewardDepositInfo.vals()) {
                    if (reward.depositTime >= timeThreshold and reward.depositTime > deposit.lastClaimedTime and reward.depositTime > deposit.startTime) {
                        let currentMul = await getCurrentMultiplierBaseOnTimestamp(deposit, reward.depositTime);
                        let withdrawValue = Float.fromInt(reward.amount) * (currentMul / Float.fromInt(reward.totalShare));
                        totalAmount += Nat64.toNat(Int64.toNat64(Float.toInt64(withdrawValue)));
                    };
                };
            };
        };
    ...
    public shared (msg) func depositReward(tokenId : Principal, value : Nat) : async TxReceipt {
        ...
        if (not tokens.hasToken(tid)) return #err("Token does not exist");
        ...
        try {
            ...
            // Limit the size of rewardDepositInfo
